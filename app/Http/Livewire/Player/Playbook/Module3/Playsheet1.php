...
class Playsheet1 extends Component
{
    ...
    public function mount($playbook_id){
        ...
        if($this->playbook->response){
            ...
            if(isset($response['module3']) && isset($response['module3']['playsheet'.$this->playsheet])){
                $this->home = $response['module3']['playsheet'.$this->playsheet]['home'];
                $this->school = $response['module3']['playsheet'.$this->playsheet]['school'];
                $this->activity = $response['module3']['playsheet'.$this->playsheet]['activity'];
                $this->other = $response['module3']['playsheet'.$this->playsheet]['other'];
            }
        }
    }
    ...
    {
        return view('livewire.player.playbook.module3.playsheet1');
    }
}
