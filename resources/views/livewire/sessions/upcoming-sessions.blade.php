<div class="sessions-page">
    <div class="row">
            ...
            <div class="card w-100 p-3 my-3">
                ...
                <div class="row mt-3">
                    <div class="col-12 d-flex justify-content-end">
                        @if(auth()->user()->user_type_id == 2 && $session->coaches()->where('coach_id', auth()->user()->coach->id)->first())
                        <button class="btn btn-danger text-white" wire:click="leaveSession({{$session->id}})">{{__('Leave Session')}}</button>
                        @elseif(auth()->user()->user_type_id == 2)
                        <button class="btn btn-dark text-white" wire:click="joinSession({{$session->id}})">{{__('Join Session')}}</button>
                        @endif
                    </div>
                </div>
                @endif
        ...
        <div class="col-xl-3 position-relative">
            <div class="card w-100 p-3 my-3 sticky-xl-top">
                <h2>{{__('Check The Schedule')}}</h2>
                <p>{{__('This is floating event schedule section you can check the details about time duration, speaker, venue & more.')}}</p>
                <h5 class="fw-bold">{{__('This week sessions')}}</h5>
                <div class="row">
                    <div class="col-12 d-flex flex-column justify-content-center align-items-center">
                        <button class="btn my-2 week-btn {{$week_day == 'Monday' ? 'btn-dark' : 'btn-light'}}" wire:click="updateWeekDay('Monday')">{{__('Monday')}}</button>
                        <button class="btn my-2 week-btn {{$week_day == 'Tuesday' ? 'btn-dark' : 'btn-light'}}" wire:click="updateWeekDay('Tuesday')">{{__('Tuesday')}}</button>
                        <button class="btn my-2 week-btn {{$week_day == 'Wednesday' ? 'btn-dark' : 'btn-light'}}" wire:click="updateWeekDay('Wednesday')">{{__('Wednesday')}}</button>
                        <button class="btn my-2 week-btn {{$week_day == 'Thursday' ? 'btn-dark' : 'btn-light'}}" wire:click="updateWeekDay('Thursday')">{{__('Thursday')}}</button>
                        <button class="btn my-2 week-btn {{$week_day == 'Friday' ? 'btn-dark' : 'btn-light'}}" wire:click="updateWeekDay('Friday')">{{__('Friday')}}</button>
                        <button class="btn my-2 week-btn {{$week_day == 'Saturday' ? 'btn-dark' : 'btn-light'}}" wire:click="updateWeekDay('Saturday')">{{__('Saturday')}}</button>
                        <button class="btn my-2 week-btn {{$week_day == 'Sunday' ? 'btn-dark' : 'btn-light'}}" wire:click="updateWeekDay('Sunday')">{{__('Sunday')}}</button>
                    </div>
                </div>
            </div>
        </div>
